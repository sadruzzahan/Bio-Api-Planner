...
const COMPARE_METRICS = [
  { key: "hrv_rmssd", label: "HRV", color: "hsl(var(--primary))" },
  { key: "resting_hr", label: "Resting HR", color: "hsl(188 60% 55%)" },
  { key: "spo2", label: "SpO2", color: "#22c55e" },
  { key: "recovery_score", label: "Recovery", color: "#f59e0b" },
];

export default function Biometrics() {
  const [selectedMetric, setSelectedMetric] = useState<string>("hrv_rmssd");
  const [rangeDays, setRangeDays] = useState<number>(30);
  const [compareMetrics, setCompareMetrics] = useState<string[]>(["hrv_rmssd", "resting_hr"]);
  ...
  return (
    ...
    <AnimatePresence>
      ...
      >
              ...
              <Card key={summary.metric} className="border-primary/20 bg-card/50" data-testid={`summary-${summary.metric}`}>
                ...
                <CardContent>
                  ...
                  <div className="mt-2 text-xs font-mono text-muted-foreground flex gap-4">
                    <span>7d: {summary.sevenDayAvg?.toFixed(1) || "-"}</span>
                    <span>30d: {summary.thirtyDayAvg?.toFixed(1) || "-"}</span>
                  </div>
                </CardContent>
              </Card>
            ))}
        ...
        <Tabs defaultValue="timeseries" data-testid="biometrics-tabs">
          <TabsList className="font-mono bg-card border border-border">
            <TabsTrigger value="timeseries" data-testid="tab-timeseries">Time Series</TabsTrigger>
            <TabsTrigger value="compare" data-testid="tab-compare">Compare Metrics</TabsTrigger>
            <TabsTrigger value="log" data-testid="tab-log">Data Log</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="timeseries" className="mt-4 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold font-mono uppercase flex items-center gap-2">
                <Activity className="w-5 h-5 text-primary" /> Last {rangeDays} Days
              </h2>
          ...
          <TabsContent value="compare" className="mt-4 space-y-4">
            <div className="flex flex-wrap gap-2" data-testid="compare-toggles">
              {COMPARE_METRICS.map(m => (
                <button
                  key={m.key}
                  onClick={() => toggleCompareMetric(m.key)}
                  className={`font-mono text-xs uppercase px-3 py-1.5 rounded-md border transition-all ${
                    compareMetrics.includes(m.key)
            ...
            <Card className="border-primary/20 bg-card/50" data-testid="compare-chart">
              <CardContent className="p-6 h-[380px]">
                {comparisonChartData.length === 0 ? (
                  <div className="flex items-center justify-center h-full text-muted-foreground font-mono">Select metrics above to compare</div>
                ) : (
                  <ResponsiveContainer width="100%" height="100%">
