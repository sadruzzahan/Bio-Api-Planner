...
export default function Biometrics() {
  ...
  const { data: entriesData, isLoading: isLoadingEntries } = useListBiometrics({ metric: selectedMetric !== "all" ? selectedMetric : undefined });
      ...
      >
            ...
            {summaryData?.slice(0, 3).map((summary) => (
              <Card key={summary.metric} className="border-primary/20 bg-card/50" data-testid={`summary-${summary.metric}`}>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-medium font-mono uppercase text-muted-foreground">{summary.metric.replace('_', ' ')}</CardTitle>
                </CardHeader>
                <CardContent>
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold font-mono uppercase flex items-center gap-2">
            <Activity className="w-5 h-5 text-primary" /> Time Series Data
          </h2>
            ...
            <TableBody>
              {isLoadingEntries ? (
                ...
                <TableRow key={entry.id} className="border-border font-mono text-sm" data-testid={`biometric-row-${entry.id}`}>
                  ...
                  <TableCell className="uppercase text-muted-foreground">{entry.metric.replace('_', ' ')}</TableCell>
